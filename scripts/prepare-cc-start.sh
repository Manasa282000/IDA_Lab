...
echo "Compute Canada Preparation Script (Singularity)"
echo "==============================================="
...
echo ${GOSDT_VERSION} > ${RESULTS_DIR}/version-gosdt.prepare
echo ${TREEBENCHMARK_VERSION} > ${RESULTS_DIR}/version-tree-benchmark.prepare

echo "---------------------------------------"

# set the slurm job file
JOB_FILE=${ROOT}/scripts/prepare-cc-slurm-job.sh
echo "slurm job file: ${JOB_FILE}"

# create the slurm logs directory

# set the outfile to be within the slurmlogs directory
OUT_FILE=${RESULTS_DIR}/prepare-slurmlog-%x-%a.out
